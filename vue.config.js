...
module.exports = {
  ...
  devServer: {
    ...
    // 配置反向代理
    proxy: {
      // 当地址中有/api的时候会触发我们的代理机制
      '/api': {
        target: 'http://ihrm.itheima.net/', // 要代理的服务器地址，这里不用写api
  ...
  configureWebpack: {
    ...
    // 打包时要排除的包名
    // 对应的是package.json里面的包名
    // key:value
    // key:要排除的包名
    // value:实际上是实际引入的包的全局变量名
    // 因为要排除element-ui 所以后面要引入CDN文件 CDN文件中有elementui的全局变量名
    // externals首先会排除掉定义的包名 空出来的位置会用变量(Vue,ELEMENT,XLSX)来替换  这三个变量是通过js引入的
    externals: externals
  },
  chainWebpack(config) {
    ...
    config.plugin('html').tap((args) => {
      // args是注入html模板的变量
      args[0].cdn = cdn // 后面的cdn就是30行定义的变量
